 ...
 */
package com.zrsf.push.xmpp.net;
...
import com.zrsf.push.jivesoftware.openfire.net.MXParser;
import com.zrsf.push.util.Config;
import com.zrsf.push.xmpp.router.PacketRouter;
...
//        session.incrementClientPacketCount();
    ...
    private void processPresence(Element doc) {
        ...
        packet.setFrom(session.getAddress());
        router.route(packet);
        if(session.getStatuspush()&&session.getStatus()!=Session.STATUS_CLOSED)//链接推送
        {
        	ShuixinService service=(ShuixinService) SpringBeanUtil.getBean("shuixinService");
        	service.pushAllNoSend(session.getAddress().getNode().toUpperCase());
//        	service.sendNoPushMessages(session.getAddress().getNode().toUpperCase());
        	session.setStatuspush(false);
        }
